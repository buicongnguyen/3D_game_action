...
describe("engagement presentation", () => {
  ...
  it("explains purchasing and active input without mixed glyphs", () => {
    expect(purchasePrice(30, 12.2)).toBe("30 scrap · need 18 more");
  ...
  it("aggregates receipts without reusing expired or different resources", () => {
    ...
    expect(receipt.text(200)).toBe("+5 scrap");
    receipt.add("weaponPart", 1, 300);
    expect(receipt.text(400)).toBe("+1 weapon parts");
    expect(receipt.text(3000)).toBe("");
    receipt.add("weaponPart", 2, 4000);
    expect(receipt.text(4100)).toBe("+2 weapon parts");
  });
});
